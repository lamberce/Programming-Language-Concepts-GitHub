			...
			((list? datum)
				(cond
					...
					[(equal? (car datum) 'let)
						(if (< (length datum) 3)
							...
							(if (not (symbol? (cadr datum)))
								(cond
									[(not (check-let-vars (cadr datum))) (eopl:error 'parse-exp "Incorrect 
											variable declarations in let" datum)]
									[else (let-exp (map parse-second (cadr datum)) (map parse-exp 
											(cddr datum)))])
								(if (symbol? (cadr datum))
									(cond
										[(not (symbol? (cadr datum))) (eopl:error 'parse-exp "Incorrect 
												name in named let" datum)]
										[(not (check-let-vars (caddr datum))) (eopl:error 'parse-exp 
												"Incorrect variable declarations in named let" datum)]
										[else (named-let-exp (cadr datum) (map parse-second 
												(caddr datum)) (map parse-exp (cdddr datum)))])	
									(eopl:error 'parse-exp "Incorrect form to let" datum))))]
					[(equal? (car datum) 'lambda)
						(cond
							[(not (>= (length datum) 3)) (eopl:error 'parse-exp "Incorrect length in 
									lambda" datum)]
							[(not (symbol-or-list-of-symbols? (cadr datum))) (eopl:error 'parse-exp 
									"Error in variable" datum)]
					...
					[(equal? (car datum) 'letrec)
						(cond
							[(< (length datum) 3) (eopl:error 'parse-exp "Incorrect length in letrec" 
									datum)]
							[(not (check-let-vars (cadr datum))) (eopl:error 'parse-exp "Error in 
									variable declarations in letrec" datum)]
							[else (letrec-exp (map parse-second (cadr datum)) (map parse-exp 
									(cddr datum)))])]
					[(equal? (car datum) 'set!)
						(cond
							[(not (= (length datum) 3)) (eopl:error 'parse-exp "Incorrect length in 
									set!" datum)]
							[(not (symbol? (cadr datum))) (eopl:error 'parse-exp "Incorrect variable 
									in set!" datum)]
							[else (set-exp (cadr datum) (parse-exp (caddr datum)))])]
					[(equal? (car datum) 'cond) (cond-exp (map list (map parse-exp 
							(map car (cdr datum))) (map parse-exp (map cadr (cdr datum)))))]
					[(equal? (car datum) 'case) (case-exp (parse-exp (cadr datum)) 
							(map list (map car (cddr datum)) (map parse-exp (map cadr (cddr datum)))))]
					[(equal? (car datum) 'begin) (begin-exp (map parse-exp (cdr datum)))]
					[(equal? (car datum) 'while) (while-exp (parse-exp (cadr datum)) 
...
(define apply-env
  (lambda (env sym succeed fail) ; succeed and fail are procedures applied if the var is or isn't found
    (begin ;(display sym)
			...
			(extended-env-record (syms vals old-env)
				(let ((pos (list-find-position var syms)))
					(if (number? pos)
						...
						(set! global-env (extended-env-record (append syms (list var)) 
								(append vals (list value)) old-env)))))
			(recursively-extended-env-record (proc-names idss bodies old-env)
				(fail)))))
	...
	(lambda (exp)
		(cases expression exp
			...
			[let-exp (var-names body)
				(app-exp (lambda-exp (map car var-names) (map syntax-expand body)) 
						(map syntax-expand (map cadr var-names)))]
			...
			[case-exp (key statements)
				(if (equal? (caar statements) 'else)
					...
					(if-exp-2 (app-exp (var-exp 'ormap) (list (lambda-exp '(z)
															(list (app-exp (var-exp 'eqv?) 
																(list (var-exp 'z) key)))) 
																	(lit-exp (caar statements))))
							  (syntax-expand (cadar statements))
			...
			[named-let-exp (name var-names body)
				(let-exp (list (list name (lambda-exp (append (map car var-names) (list name)) 
						(map (lambda (x) (add-param name x)) (map syntax-expand body))))) 
								(list (app-exp (var-exp name) (append (map syntax-expand 
										(map cadr var-names)) (list (var-exp name))))))]  
			[letrec-exp (vars body) 
				(letrec-exp (map (lambda (x) (list (car x) (syntax-expand (cadr x)))) vars) 
	...
	(lambda (add-name exp)
		(cases expression exp
			...
			[while-exp (condition body)
				(while-exp condition body)]
			[if-exp-2 (condition case1 case2)
				(if-exp-2 (add-param add-name condition) (add-param add-name case1) 
						(add-param add-name case2))]
			[if-exp-1 (condition case1)
				(if-exp-1 (add-param add-name condition) (add-param add-name case1))]
			[app-exp (rator rands)
				(if (equal? rator (var-exp add-name))
					(app-exp (add-param add-name rator) (append (map (lambda (x) 
							(add-param add-name x)) rands) (list (var-exp add-name))))
...
(define *prim-proc-names* '(+ - * add1 sub1 cons = < > <= >= / append list eq? equal? length list->vector
							list? pair? vector->list vector? number? car cdr caar cadr cdar cddr caaar 
	...
	(let ([identity-proc (lambda (x) x)])
	  (lambda (exp env)
		(cases expression exp
		  ...
		  [let-exp (var-names body)
				(eval-inorder body (extend-env (map car var-names) 
						(map (lambda (x) (eval-exp x env)) (map cadr var-names)) env))]
		  [lambda-exp (vars body)
				(closure vars body env)]
		  [letrec-exp (var-names body) 
				(eval-inorder body (extend-env-recursively (map car var-names) 
						(map cadr (map cadr var-names)) (map caaddr (map cadr var-names)) env))]
		  [while-exp (condition body) 
    ...
    (cases proc-val proc-value
	  ...
	  [closure (vars body env) 
			(if (symbol? vars)
				...
				(if (list? vars)
					(eval-inorder body (extend-env vars args env))
					(eval-inorder body (extend-env (make-vars-list-proper vars) 
							(make-args-into-proper args (length-of-improper vars)) env))))]
