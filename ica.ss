...
(define proc-replacer
	(lambda (proc vars args)
		(change-vars proc (list-shortener (map (lambda (x y) (if (list? x) (list (cadr x) y))) vars args)))))o
...
(define change-vars
	(lambda (exp li)
		(cases expression exp
			...
			[app-exp (rator rands)
				(if (equal? rator (var-exp add-name))
					(app-exp (change-vars rator li) (append (map (lambda (x) (change-vars x li)) rands)))
